...
import { Search, Loader2, Plus, ExternalLink, GitBranch, BookOpen } from "lucide-react";
      ...
      <div className="space-y-2">
        <div>
          ...
          <input
            ...
            placeholder="https://github.com/stripe/openapi/tree/master/openapi  or  https://api.example.com/specs"
        ...
        <div>
          ...
          <input
            ...
            placeholder="https://stripe.com/docs/upgrades  or  https://github.com/org/repo/blob/main/CHANGELOG.md"
            className="w-full px-2.5 py-1.5 text-xs font-mono border border-stone-200 rounded-md bg-white focus:outline-none focus:border-amber-400"
          />
        </div>
      </div>
      ...
      {results && (
        <div className="space-y-3">
          ...
          {results.versions?.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 text-[11px] font-semibold text-stone-500 uppercase tracking-wider mb-2">
                <GitBranch className="w-3 h-3" /> Discovered Spec Files ({results.versions.length})
              </div>
          ...
          {results.changelog_versions?.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 text-[11px] font-semibold text-stone-500 uppercase tracking-wider mb-2">
                <BookOpen className="w-3 h-3" /> Versions from Changelog ({results.changelog_versions.length})
              </div>
          ...
          {results.pairs?.length > 0 ? (
            ...
            <p className="text-xs text-stone-500 italic">
              {results.versions?.length > 0
                ? "Only one spec found — need at least two versions to create a comparison pair."
                : "No versioned spec files found at that URL. Try a GitHub directory URL or a versioned base path."}
            </p>
          )}
        </div>
      )}
